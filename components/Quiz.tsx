...
export const Quiz: React.FC<QuizProps> = ({ onComplete }) => {
  ...
  const [answers, setAnswers] = useState<
    { questionId: number; optionId: string; mapsTo?: SubNeedID; questionText: string; answerText: string }[]
  >([]);
  ...
  return (
    <div
      className="min-h-screen flex flex-col items-center relative overflow-hidden cosmic-texture pb-20"
      style={{
    ...
    >
      ...
      <div
        className={`flex-1 flex flex-col justify-center w-full relative z-10 px-4 py-8 md:px-6 md:py-16 transition-all duration-500 ease-out max-h-screen ${isTransitioning ? "opacity-0 translate-y-3" : "opacity-100 translate-y-0"}`}
      >
        <div className={`mx-auto ${isZodiacQuestion ? "max-w-3xl" : "max-w-lg"} w-full`}>
          ...
          <div className="mb-6 md:mb-10 bg-cosmic-400/50 rounded-2xl p-5 md:p-10 cosmic-shadow mystical-border backdrop-blur-md">
            ...
            <h2 className="text-xl md:text-3xl font-serif text-white leading-relaxed">{currentQuestion.question}</h2>
          </div>

          {/* Options */}
          <div
            className={`${isZodiacQuestion ? "grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4" : "flex flex-col gap-3 md:gap-4"} max-h-[50vh] overflow-y-auto`}
          >
            {currentQuestion.options.map((option, idx) => (
              ...
              >
                {isZodiacQuestion ? (
                  // --- ZODIAC STYLE ---
                  <>
                    <span className="font-serif text-2xl md:text-3xl text-gold-300 mb-2 md:mb-3 group-hover:text-gold-200 transition-colors group-hover:scale-110 transform duration-300">
                      {option.symbol}
                    </span>
                    <span className="block font-serif text-sm md:text-base text-white mb-1 group-hover:text-gold-100 transition-colors">
                      {option.text}
                    </span>
                  ...
                  <>
                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-cosmic-300/60 mystical-border flex items-center justify-center flex-shrink-0 group-hover:bg-gold-500/40 group-hover:border-gold-400 transition-all">
                      <span className="text-xs md:text-sm font-semibold text-gold-300 group-hover:text-gold-200">
